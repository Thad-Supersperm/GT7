...
function renderTable(carData, priceMap, user) {
    ...
    for (const car of carData) {
        ...
        let priceCellsHtml = ''; let lastUsedPrice = 'initial'; let lastLegendPrice = 'initial'; let colspan = 0;
        const appendCell = () => { if (colspan === 0) return; let cellContent = ''; if (lastUsedPrice) cellContent += `<div class='price-used'>U: ${parseInt(lastUsedPrice).toLocaleString()}</div>`; if (lastLegendPrice) cellContent += `<div class='price-legend'>L: ${parseInt(lastLegendPrice).toLocaleString()}</div>`; priceCellsHtml += `<td class="price-cell" colspan="${colspan}">${cellContent}</td>`; };
        for (const dateStr of calendarDates) { const currentUsed = priceMap[`${car.id},${dateStr},used`] || null; const currentLegend = priceMap[`${car.id},${dateStr},legend`] || null; if (currentUsed === lastUsedPrice && currentLegend === lastLegendPrice) { colspan++; } else { appendCell(); lastUsedPrice = currentUsed; lastLegendPrice = currentLegend; colspan = 1; } }
        appendCell(); row.insertAdjacentHTML('beforeend', priceCellsHtml);
    }
